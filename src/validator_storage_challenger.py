...
class ValidatorStorageChallenger:
    ...
    def accept_challenge(
        ...
        accused_signal = (
            ReliabilitySignal.CHALLENGE_SUCCEEDED
            if passed
            else ReliabilitySignal.CHALLENGE_FAILED
        )
        reporter_signal = (
            ReliabilitySignal.VALIDATOR_UPHELD_REPORT
            if passed
            else ReliabilitySignal.VALIDATOR_DISMISSED_REPORT
        )

        accused_result = self.reliability_manager.apply_signal(
            accused_partner, accused_signal, job_id
        )
        reporter_result = self.reliability_manager.apply_signal(
            reporter_partner, reporter_signal, job_id
        )
...
if __name__ == "__main__":
    ...
    validator = ValidatorStorageChallenger(
        validator_id="VALIDATOR-001", reliability_manager=ReliabilityManager()
    )
